...
def train_gmm(data_label, features, train_keys, train_folders, audio_ext, dict_file, ncomp, init_only=False):
    ...
    if exists(partial_gmm_dict_file):
        ...
        for k, train_key in enumerate(train_keys):
            ...
            #print(pd)
            files = pd[pd[1] == data_label][2]
            #print('------')
            #print(files)
            # files_subset = sample(list(files), 1000)  # random init with 1000 files
            files_subset = (files.reset_index()[2]).loc[list(range(0, len(files), 10))]  # only every 10th file init
            for file in files_subset:
                # print('-----')
                # print(train_folders[k])
                # print(file)
                try:
                    Tx = extract_features(file, features=features, cached=True)
                    data.append(Tx.T)
                    #print(Tx.T)
                except:
                    print(file)
...
def scoring(scores_file, dict_file, features, eval_ndx, eval_folder, audio_ext, features_cached=True, flag_debug=False):
    ...
    #files = "/data3/mahaoxin/xule/Output/track32test/"+pd[0].values #+pd[2].values
    files = pd[2].values
    # print(files)
    ...
    for i, file in enumerate(files):
        ...
        Tx = extract_features(file, features=features, cached=False)#features_cached)
        try:            
